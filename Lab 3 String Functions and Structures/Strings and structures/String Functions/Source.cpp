...
char* strtok_s(char*, char*);
const char* strstr_s(char*, const char*);
//-------------------------------------------------------\\
...
int main()
{
	...
	while (t == 'y') 
	{
		...
		switch (x) {
			...
			{
					str1=new char[50];
					cout << "\n Enter a string you want to break into tokens having dots, commas and _ sperating the words.\n    ";
					cin >> str1;
					//cout<<str1;
					str2=strtok_s(str1,".,_");
					cout << endl;
					while(str2 != NULL)
					{
						cout<<" "<<str2<<endl;
...
// Function for converting a string into tokens...
char* strtok_s(char* str1, char* delimeter)
{
	static int movePointer=0;
	...
	while(str1[i]!='\0')
	{
		while (delimeter[j] != '\0')
		{
			if (str1[i] == delimeter[j])
			{
				str2[i] = '\0';
				movePointer += (strlen_s(str2) + 1);
				return str2;
			}
			j++;
		}
...
// Function for finding a string in an another string...
const char* strstr_s( char* str1, const char* str2)
{
	char* temp;
	int movePointer = 0;
	temp = strtok_s(str1, " ");
